...
class HtmlController extends Admin {
    ...
    public function __construct() {
	    ...
	    $this->assign('ck_ob', $ck_ob_function);
	    $this->assign('ismb', $this->site['SITE_MOBILE']);
	}
	...
	public function indexAction() {
	    return $this->display('admin/html_list');
	}
	...
	public function categoryAction() {
	    if ($this->isPostForm()) {
			...
			if ($this->site['SITE_EXTEND_ID']) {
                return $this->adminMsg(lang('a-sit-29', array('1' => $sites[$this->site['SITE_EXTEND_ID']]['SITE_NAME'])));
            }
			foreach ($sites as $sid => $t) {
				if ($t['SITE_EXTEND_ID'] && $t['SITE_EXTEND_ID'] == $this->siteid) {
					return $this->adminMsg(lang('a-sit-30', array('1' => $t['SITE_NAME'])));
				}
			}
			return $this->adminMsg(lang('a-cat-101'), url('admin/html/category', array('submit' => 1, 'catid' => $catid, 'isall' => $isall)), 0, 1, 2);
		}
		...
		if ($submit) {
			...
			if ($catid && $isall == 0) {
				...
				if (isset($cats[$key]) && $this->cats[$cats[$key]]) {
				    ...
				    $this->cache->delete('html_cats');
				    return $this->adminMsg(lang('a-con-107') . '(' . $count . ')', '', 0, 1, 1);
				}
			} else {
				if (empty($catid)) {
				    $cats = $this->cats;
	        ...
	        $this->assign('category_select', $this->tree->get_tree($this->cats, 0));
	        return $this->display('admin/html_create');
		}
	}
	...
	public function showAction() {
		if ($this->isPostForm()) {
			$this->cache->delete('html_cats');
			return $this->adminMsg(lang('a-cat-101'), url('admin/html/show', array('submit' => 1, 'catid' => $this->post('catid'), 'totime' => $this->post('totime'))), 0, 1, 2);
		}
		$submit = (int)$this->get('submit');
		if ($submit) {
			$page = $this->get('page') ? $this->get('page') : 1;
			...
			if (empty($tohtml)) {
				$this->cache->delete('html_cats');
				return $this->adminMsg(lang('a-mod-143'));
			} else {
				$cats = implode(',', $tohtml);
				$this->cache->set('html_cats', $cats);
	...
	public function formAction() {
		if ($this->isPostForm()) {
			return $this->adminMsg(lang('a-cat-101'), url('admin/html/form', array('submit' => 1, 'mid' => $this->post('mid'))), 0, 1, 2);
		}
		$submit	= (int)$this->get('submit');
		...
		if ($submit) {
			...
			if (!isset($form[$mid]['tablename'])) {
                return $this->adminMsg(lang('a-cat-109', array('1' => $mid)));
            }
			if (!isset($form[$mid]['setting']['form']['url']['tohtml']) || empty($form[$mid]['setting']['form']['url']['tohtml'])) {
				return $this->adminMsg(lang('a-cat-110'));
			}
			...
			if ($page >= $totalpage) {
				...
				return $this->adminMsg('【' . $form[$mid]['modelname'] . '】(' . $page.'/' . $totalpage .')', url('admin/html/form',array('page' => $nextpage, 'mid' => $mid, 'submit' => 1,'count' => $count)), 3, 1, 2);
			}
		} else {
	        $this->assign('list', $form);
	        return $this->display('admin/html_form');
		}
	}
	...
	public function indexcAction() {
		if ($this->site['SITE_MOBILE']) {
			return $this->adminMsg(lang('dr014'));
		}
		ob_start();
		$this->assign(array(
	        'indexc' => 1,
		...
		return $this->adminMsg(lang('a-con-107') . '(' . formatFileSize($size) . ')', '', 3, 1, 1);
	}
	...
	public function clearAction() {
	    $submit	= (int)$this->get('submit');
		if (empty($submit)) {
            return $this->adminMsg(lang('a-con-108'), url('admin/html/clear', array('submit' => 1)), 3, 1, 2);
        }
	    @unlink('index.html');
		$htmlfiles	= $this->cache->get('html_files');
		if (empty($htmlfiles)) {
            return $this->adminMsg(lang('a-con-109'), '', 3, 1, 1);
        }
	    ...
	    return $this->adminMsg(lang('a-con-110', array('1' => $d, '2' => $f)), '', 3, 1, 1);
	}
	...
	private function toCategory($catid, $page, $isall, $key, $filecount) {
		...
		if ($cat['setting']['url']['tohtml'] == 1) {
			if (($cat['child'] && $cat['categorytpl'] != $cat['listtpl']) || $cat['typeid'] == 2) {
				...
				$total = $this->content->count(null, '`status`=1 AND `catid` IN (' . $cat['arrchilds'] . ')', null, 3600);
	    ...
	    return $this->adminMsg('【' . $cat['catname'] . '】(' . $page . '/' . $totalpage . ')', url('admin/html/category', array('page' => $nextpage, 'catid' => $nextcatid, 'isall' => $isall, 'key' => $key, 'filecount' => $filecount, 'submit' => 1)), 0, 1, 2);
	}
	...
	private function toContent($cats, $page, $filecount, $totime = 0) {
		...
		$total = $this->content->count(null, $where, null, 3600);
		$pagesize = 50;
	    $totalpage = ceil($total/$pagesize);
        $data = $this->content->page_limit($page, $pagesize)->where($where)->order('id ASC')->select();
		if (empty($data)) {
			$this->cache->delete('html_cats');
			return $this->adminMsg(lang('a-con-107') . '(' . $filecount . ')', '', 0, 1, 1);
		}
		foreach ($data as $t) {
        ...
        return $this->adminMsg(lang('a-con-111') . " ($page/$totalpage)", url('admin/html/show', array('page' => $page+1, 'submit' => 1, 'filecount' => $filecount, 'totime' => $totime)), 0, 1, 2);
	}
