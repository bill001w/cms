...
{
    $cache = new \App\Services\dayrui\libraries\cache_file();
 ...
 * 读取模型缓存数据(非会员)
 */
function get_model_data($name = 'content')
{
    $cache = new \App\Services\dayrui\libraries\cache_file();
    $data = $cache->get('model_' . $name);
...
function word2pinyin($word)
{
    ...
    $pin = new \App\Services\dayrui\libraries\pinyin();
...
{
    return \App\Services\libraries\auth::check($roleid, $action, 'admin');
}
