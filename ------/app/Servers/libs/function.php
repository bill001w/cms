...
function get_category_data($site = 0) {
	$cfg  	= \App\Servers\drivers\App::get_config();
	$cache 	= new \App\Servers\dayrui\libraries\cache_file();
	$site	= empty($site) ? \App\Servers\drivers\App::get_site_id() : $site;
 ...
 * 读取模型缓存数据(非会员)
 */
function get_model_data($name = 'content', $site = 1) {
	$cache = new \App\Servers\dayrui\libraries\cache_file();
	$data = $cache->get('model_' . $name . '_1');
...
function sitemap_xml() {
	...
	$db = ControllerTool::model('content_' . App::get_site_id());
...
function get_member_avatar($uid, $size = 90) {
    ...
    $db    = ControllerTool::model('member');
	$cache = new cache_file();
...
function get_member_info($uid, $more = 0) {
	...
	if ($more) { //会员附表
		...
		if (isset($model[$data['modelid']])) {
		    $d    = ControllerTool::model($model[$data['modelid']]['tablename']);
			$r    = $d->find($uid);
...
function word2pinyin($word) {
    ...
    $pin = new \App\Servers\dayrui\libraries\pinyin();
...
function admin_auth($roleid, $action) {
	return \App\Servers\dayrui\libraries\auth::check($roleid, $action, 'admin');
}
