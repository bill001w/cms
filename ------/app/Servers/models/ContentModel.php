...
class ContentModel extends Model
{
    ...
    {
        return $this->count('content_1', null, $where, $value, $cache);
    }
    ...
    private function download_images($imgs, $uid = 0, $size = 0)
    {
        ...
        if ($config['SITE_WATERMARK']) $image = ControllerTool::instance('image_lib');
        foreach ($imgs as $img) {
            ...
            if (file_put_contents($name, $content)) {
                if ($config['SITE_WATERMARK']) {
                    $image = ControllerTool::instance('image_lib');
