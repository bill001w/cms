...
class mysql {
	...
	public function __construct($params) {
		if (!$params['host'] || !$params['username'] || !$params['dbname']) ControllerTool::halt('Mysql数据库配置文件不完整'); //检测参数信息是否完整
		...
		if (!$this->db_link) ControllerTool::halt('Mysql服务器(' . $params['host'] . ')连接失败 <br/>Error Message:' . (function_exists('iconv') ? iconv('GBK', 'UTF-8', mysql_error()) : mysql_error()) . '<br/>Error Code:' . mysql_errno(), 'Warning');
		if (!mysql_select_db($params['dbname'], $this->db_link)) ControllerTool::halt('不能连接到数据库<br/>' . (function_exists('iconv') ? iconv('GBK', 'UTF-8', mysql_error()) : mysql_error()) . ' Error Message:' . mysql_error(), 'Warning');
	...
	public function query($sql, $link = null) {
		...
		if (SYS_DEBUG === false) {
			...
			if ($result == true) {
				...
				ControllerTool::halt($namespace_code . '[' . $controller_id . '][' . $action_id . '] SQL execute failed :' . $sql . '<br/>Error Message:' . $this->error() . '<br/>Error Code:'.$this->errno(). '<br/>Error SQL:' . $sql);
			} 
		}
		return $result;
	}
