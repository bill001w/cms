...
abstract class Model
{
    ...
    {
        $ci = &get_instance();
        $params = require CONFIG_DIR . 'database.ini.php';
        $this->db = $ci->db;
    ...
    protected function get_table_name()
    {
        if (!$this->table_name) {
            // 根据model类名获取表名
            $model_id = trim(strrchr(substr(strtolower(get_class($this)), 0, -5), DIRECTORY_SEPARATOR), DIRECTORY_SEPARATOR);
    ...
    public function select($all_data = true, $cache = 0)
    {
        if (!isset($this->_parts['from']) || !$this->_parts['from']) {
            $this->from($this->table_name ? substr($this->table_name, strlen($this->prefix)) : trim(strrchr(substr(strtolower(get_class($this)), 0, -5), '\\'), '\\'));
        }
        //组装完整的SQL查询语句
    ...
    public function load_model($model_name)
    {
        ...
        return ControllerTool::model($model_name);
    }
