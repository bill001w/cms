...
namespace App\Servers\Core;

use App\Servers\models\ContentModel;
use App\Servers\models\FormModel;
...
abstract class App
{
        ...
        if (isset(self::$core_class_array[$class_name])) {
            ...
            if (is_file(MODEL_DIR . $class_name . '.php')) {
                ...
                if (strpos($class_name, 'Content_') === 0 || strpos($class_name, 'Form_') === 0 || strpos($class_name, 'Member_') === 0) {
                    ...
                    ControllerTool::halt('The Model file: ' . $class_name . ' is not exists!');
                }
            }
        } else {
    ...
    public static function singleton($class_name)
    {
        ...
        if (strpos($key, 'content_') === 0) {
            ...
            $class_name = 'App\\Servers\\models\\' . $class_name;
    ...
    public static function plugin_model($plugin, $table_name)
    {
        ...
        if (!is_file($model_file)) ControllerTool::halt('The pluginModel(#' . $plugin . ') file:' . $model_name . '.php is not exists!');
    ...
    public static function load_file($file_name)
    {
        ...
        if (!isset(self::$_inc_files[$file_name]) || self::$_inc_files[$file_name] == false) {
            if (!is_file($file_name)) ControllerTool::halt('The file:' . $file_name . ' not found!');
