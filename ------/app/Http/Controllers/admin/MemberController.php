...
//            $modelid = (int)$request->get('modelid');
    ...
    public function editAction(Request $request)
    {
        ...
        if ($this->isPostForm()) {
            ...
            if ($request->method() === 'POST' && $request->get('password')) $data['password'] = md5(md5($request->method() === 'POST' && $request->get('password')) . $member['salt'] . md5($request->method() === 'POST' && $request->get('password')));
            foreach ($data as $i => $t) {
...
//        $type = $request->get('type');
    ...
    public function extendAction(Request $request)
    {
        ...
        switch ($type) {
            case 'set':    //参数设置
                if ($this->isPostForm()) {
                    $cfg = $request->method() === 'POST' && $request->get('setting');
                    $data = $request->method() === 'POST' && $request->get('data');
                    $field = array();
                    if ($cfg['member']['field']) {
                    ...
                    $this->adminMsg($this->getCacheCode('model') . lang('success'), url('admin/member/extend', array('modelid' => $mid, 'type' => 'set', 'touserid' => $touser, 'typeid' => $request->method() === 'POST' && $request->get('typeid'))), 3, 1, 1);
                }
            ...
            case 'edit':    //修改
                ...
                break;
            default:    //列表数据
                if ($this->isPostForm() && $request->method() === 'POST' && $request->get('form') == 'search') {
                    $kw = $request->method() === 'POST' && $request->get('kw');
                    $stype = $request->method() === 'POST' && $request->get('stype');
                    $userid = (int)$request->method() === 'POST' && $request->get('userid');
                    $touser = (int)$request->method() === 'POST' && $request->get('touserid');
                } elseif ($this->isPostForm() && $request->method() === 'POST' && $request->get('form') == 'del' && $request->method() === 'POST' && $request->get('ids')) {
                    $ids = implode(',', $request->method() === 'POST' && $request->get('ids'));
                    $table->delete('id IN(' . $ids . ')');
                } elseif ($this->isPostForm() && $request->method() === 'POST' && $request->get('form') == 'status_1' && $request->method() === 'POST' && $request->get('ids')) {
                    $ids = implode(',', $request->method() === 'POST' && $request->get('ids'));
                    $table->update(array('status' => 1), 'id IN(' . $ids . ')');
                } elseif ($this->isPostForm() && $request->method() === 'POST' && $request->get('form') == 'status_0' && $request->method() === 'POST' && $request->get('ids')) {
                    $ids = implode(',', $request->method() === 'POST' && $request->get('ids'));
                    $table->update(array('status' => 0), 'id IN(' . $ids . ')');
                } elseif ($this->isPostForm() && $request->method() === 'POST' && $request->get('form') == 'status_3' && $request->method() === 'POST' && $request->get('ids')) {
                    $ids = implode(',', $request->method() === 'POST' && $request->get('ids'));
    ...
    {
        $email = $request->method() === 'POST' && $request->get('email');
        if (!check::is_email($email)) exit('<b><font color=red>' . lang('a-mem-21') . '</font></b>');
        $id = $request->method() === 'POST' && $request->get('id');
    ...
    {
        $name = $request->method() === 'POST' && $request->get('username');
