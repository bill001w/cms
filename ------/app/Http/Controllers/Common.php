...
use App\Servers\Core\App;
use App\Servers\Core\database\drivers\mysqli\CI_DB_mysqli_driver;
use App\Servers\Core\View;
use App\Servers\dayrui\libraries\cache_file;
use App\Servers\drivers\ControllerTool;
use App\Servers\models\CategoryModel;
use App\Servers\models\ContentModel;
use App\Servers\models\MemberModel;
use Illuminate\Http\Request;
use App\Servers\Core\Lang;
...
//        }
        ...
        define('IS_ADMIN', true/*$this->session->userdata('user_id')*/);
...
//        if ($this->namespace != 'admin') $tpl = '../' .
    ...
    {
        require_once(app_path() . '/Servers/extensions/fields.php');
...
//dd($data_fields);
    ...
    private function _init()
    {
        ...
        define('APP_ROOT', app() . DIRECTORY_SEPARATOR . 'Servers' . DIRECTORY_SEPARATOR);
        ...
        $config['SITE_BOTTOM_INFO'] = @html_entity_decode($config['SITE_BOTTOM_INFO']);

        $config = require APP_ROOT . 'Config/database.ini.php';
        ...
        App::$config = $this->config = $config;
        App::$language = $this->language = $language;
        App::$db = $this->db = new CI_DB_mysqli_driver($config);
        App::$lang = new Lang();
    }
